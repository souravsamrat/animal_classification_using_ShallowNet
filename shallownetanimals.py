...
import imagepreprocessor as ir
import shallownet as snet
...
sp = ir.SimplePreprocessor(32,32)
iap = ita.ImageToArrayPreprocessor()
sdl = dl.SimpleDatasetLoader(preprocessors = [sp,iap])
...
opt = SGD(lr = 0.005)
model = snet.ShallowNet.build(width = 32, height = 32, depth = 3, classes = 3)
